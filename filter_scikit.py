...
pos_data, pos_label = load_data("data/pos/en", 1)
print len(pos_data)
neg_data, neg_label = load_data("data/neg/en", 0)
print len(neg_data)
...
for train_index, test_index in sss.split(all_data, all_labels):
    ...
    vectorizer = TfidfVectorizer(sublinear_tf=True, max_df=0.5, stop_words='english', max_features=40000)
    ...
    model = Sequential()
    model.add(Dense(4096, activation='relu', input_shape=(X_train_fea.shape[1],)))
    ...
    with open(''.join(('tfidf', str(idx), 'en.pkl')), 'w') as f:
